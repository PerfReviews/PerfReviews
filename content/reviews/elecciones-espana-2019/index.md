...
Lo primero que nos llama la atención de la web de Vox es el gran número de recursos bloqueantes definidos en el `<head>` del documento HTML, 12 peticiones de CSS y 11 de Javascript, entre las que podemos ver los widgets de Facebook y Twitter. Todo esto hace que el TTI (Time To Interactive), es decier el tiempo de espera del usuario para poder interactuar con el sitio web, se dispare a tiempos de espera demasiado altos. En cuanto a las imágenes, tienen un gran marge de mejora en la optimización y la malloría se sestán representando a un tamaño inferior al de la imagen original. A primera vista, por el nombre la url, parece que los estáticos están versionados, pero no tienen caché, cosa que mejoraría la experiencia de usuario definiendo un tiempo de caché adecuado.
